...
import { PlayerInterface } from "./Equipe";
...
export default function Saison() {
  const [gamesData, setGamesData] = useState<GameInterface[] | null>(null);
                ...
                <div className="flex justify-between w-3/6 border-gray-100 border-l-2 border-r-2 border-b-2 rounded-b-xl drop-shadow-sm ">
                  <div className="w-1/3 p-6">
                    ...
                    <div className="text-center  ">
                      <div className="my-4">{game.homeTeamId}</div>
                      {playersData &&
                        playersData.map((player) =>
                          player.id === game.homeTeamId ? (
                            <div key={player.id}>
                  ...
                  <div className="w-1/3 p-6">
                    ...
                    <div className="text-center">
                      <div className="my-4">{game.awayTeamId}</div>
                      {playersData &&
                        playersData.map((player) =>
                          player.id === game.awayTeamId ? (
                            <div key={player.id}>
                              {player.firstname} {player.lastname} -{" "}
                              {player.position}
                            </div>
